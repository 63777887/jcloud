        ...
        return new AbstractDataSourceProvider() {
            ...
            public Map<String, DataSource> loadDataSources() {
                ...
                if (null != shardingDataSource) {
                    ShardingDataSource dataSource = shardingDataSource
                        .unwrap(ShardingDataSource.class);
                    dataSourceMap.put(SHARDING_DATA_SOURCE_NAME, dataSource);
                    //打开下面的代码可以把 shardingjdbc 管理的数据源也交给动态数据源管理 (根据自己需要选择开启)
                    dataSourceMap.putAll(((ShardingConnection) shardingDataSource.getConnection())
                        .getDataSourceMap());
                }
