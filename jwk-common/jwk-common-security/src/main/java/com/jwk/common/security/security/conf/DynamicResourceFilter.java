...
public class DynamicResourceFilter extends AbstractSecurityInterceptor implements Filter {
	...
	/**
	 * 查询用户所有资源 查询当前请求路径资源 然后比较路径资源是否在用户资源列表中
	...
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain)
		...
		if (noAuth.get()) {
			...
			// 拿到了 Authentication，这里是调用 authenticateIfRequired方法拿到了，其实里面还是通过
			// SecurityContextHolder拿到的
			// 调用了 accessDecisionManager.decide(authenticated, object, attributes)，前两步都是对
			// decide方法做参数的准备，第三步才是正式去到鉴权的逻辑，既然这里面才是真正鉴权的逻辑，那也就是说鉴权其实是
			// accessDecisionManager在做。
