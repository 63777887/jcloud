...
public class JwkTokenEndpoint {
    ...
    public RestResponse<Boolean> logout(
            ...
            @RequestHeader(value = OAuth2ParameterNames.REFRESH_TOKEN, required = false) String refreshToken) {
        if (StrUtil.isBlank(authHeader)) {
            return RestResponse.success();
        }

        String tokenValue = authHeader.replace(OAuth2AccessToken.TokenType.BEARER.getValue(), StrUtil.EMPTY).trim();
        return removeToken(tokenValue, refreshToken);
    }
    ...
    public RestResponse<Boolean> removeToken(@RequestParam("token") String token,
        ...
        Long orgIg = MapUtil.getLong(claims, JwkSecurityConstants.DETAILS_ORGID);
        String clientId = MapUtil.getStr(claims, JwkSecurityConstants.CLIENT_ID);
        // 清除token记录
        upmsRemoteService.removeToken(RemoveTokenDto.builder().orgId(orgIg).userId(userId).accessToken(token).refreshToken(refreshToken).build());
        // 处理自定义退出事件，保存相关日志
