...
public class AuthServiceImpl implements AuthService {
	...
	private void checkUserExist(String phone, String username, String email) {
		List<SysUser> allUser = sysUserService.lambdaQuery().and(tmp -> tmp.eq(SysUser::getPhone, phone).or()
				.eq(SysUser::getUsername, username).or().eq(SysUser::getEmail, email)).list();
		allUser.forEach(t -> {
	...
	public Boolean registerImportUsers(List<UserImportDto> datalist) {
		...
		List<String> orgNames = userlist.stream().map(UserImportDto::getOrgName).filter(StrUtil::isNotBlank).distinct()
				.collect(Collectors.toList());
	...
	private UserInfo getUserInfo(SysUser user) {
		...
		for (Long roleId : roleIds) {
			// 通过用户角色列表加载用户的资源权限列表
			List<SysRoleMenu> sysRoleMenuList = sysRoleMenuService.lambdaQuery().eq(SysRoleMenu::getRoleId, roleId)
					.list();
